package com.leimingtech.extend.module.payment.module.alipay.pc.china.pay.controller;
...
public class AplipayAction {
	...
	public void toPay(HttpServletRequest request ,HttpServletResponse response){
		...
		paycommon.setOutTradeNo("P20151116105435114258340");
		paycommon.setPayAmount(BigDecimal.valueOf(0.01d));
		paycommon.setNotifyUrl("3434");
		try {
			String sHtmlText = "";
			if(StringUtils.isEmpty(orderSn)){
				sHtmlText = AlipaySubmit.errorRedirect(null, null);//构造错误跳转方法
			}else{
				sHtmlText = alipayService.toPay(paycommon);//构造提交支付宝的表单
			}
	...
	* 如果没有收到该页面返回的 success 信息，支付宝会在24小时内按一定的时间策略重发通知
	...
	public void BackReturn(HttpServletRequest request ,HttpServletResponse response){
		...
		try {
			...
			payMap = alipayService.payback(request);
